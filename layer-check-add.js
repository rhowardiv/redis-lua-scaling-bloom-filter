...
var addsource   = fs.readFileSync('layer-check-add.lua', 'ascii');
...
function add(n, layer, id) {
  if (n == count) {
    ...
    console.log((found / ((count * (layers / 2)) / 100)) + '% false positives');
  ...
  var maxlayer = 1 + Math.floor(n / layersize);
  if (!layer) {
    layer = 1;
  }

  client.evalsha(addsha, 0, 'test', entries, precision, id, 600, function(err, exists) {
    if (err) {
      throw err;
    }

    if (exists && exists == layer) {
      ++found;
    }
